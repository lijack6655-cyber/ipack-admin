...
import { emptyProduct, productInput, publishIssues, type ProductInput } from '@/lib/products/model';
...
type Media = { id:string; path:string; name:string; width:number; height:number };
...
function ProductEditor() {
  ...
  const change = <K extends keyof ProductInput>(key:K,value:ProductInput[K]) => { setForm(previous => ({...previous,[key]:value})); setConfirmed(false); setNotice(''); setPreview(''); };
  ...
  const input=(key:keyof ProductInput,label:string,placeholder='',multiline=false)=><label className="block text-sm font-medium text-slate-700" key={key}>{label}{multiline?<textarea className={`${fieldClass} mt-1`} rows={key==='description'?10:3} value={String(form[key])} onChange={e=>change(key,e.target.value)} placeholder={placeholder}/>:<input className={`${fieldClass} mt-1`} value={String(form[key])} onChange={e=>change(key,e.target.value)} placeholder={placeholder}/>}</label>;
  if (!user) return null;
  return <AdminLayout user={user} onLogout={async()=>{if(!dirty||window.confirm('未保存的修改会丢失，确定退出吗？')){bypassNavigation.current=true;await logout();await router.push('/login');}}}>
    <div className="flex flex-wrap items-center justify-between gap-3 mb-6"><div><h1 className="text-2xl font-bold">{id?'编辑产品':'新建产品'}</h1><p className="text-sm text-slate-500 mt-1">完善产品资料、选择图片，预览后发布到网站。</p></div><button className={buttonClass} onClick={()=>router.push('/admin/products')}>返回产品列表</button></div>
    ...
    {loading?<p>正在读取产品资料…</p>:<fieldset disabled={busy||Boolean(id&&!product)} className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_320px] gap-6 min-w-0">
      <div className="space-y-6 min-w-0"><section className={panelClass}><h2 className="font-semibold">产品信息</h2>{input('title','产品名称 *','填写对外展示的英文产品名称')}
        <div className="grid sm:grid-cols-2 gap-4">{input('sku','SKU / 内部货号','发布前必填')}<label className="block text-sm font-medium text-slate-700">产品分类<select aria-label="产品分类" className={`${fieldClass} mt-1`} value={form.category_id} onChange={e=>change('category_id',e.target.value)}><option value="">选择分类</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label></div>
      ...
      </section><section className={panelClass}><h2 className="font-semibold">产品介绍</h2>{input('short_description','简短描述','显示在详情页标题下方',true)}{input('description','详细描述','填写实际用途、适配条件及买家需要确认的信息。空行分段。',true)}
      ...
      <aside className="space-y-6 min-w-0"><section className={panelClass}><h2 className="font-semibold">发布</h2><p className="text-sm">状态：<strong>{product?.status==='published'?'已发布':product?.status==='archived'?'已下架':'草稿'}</strong>{hasDraft&&product?.status==='published'&&' · 有待发布草稿'}</p><p className="text-xs text-slate-500">{dirty?'有未保存修改':hasDraft?'草稿已保存':'当前内容已载入'}</p>
        <div className="grid grid-cols-2 gap-2"><button type="button" className={buttonClass} onClick={()=>action('save')}>保存草稿</button><button type="button" className={buttonClass} disabled={!hasDraft||dirty} onClick={()=>action('preview')}>预览草稿</button></div>
        {input('verification_note','资料核验说明','填写资料来源及已核对的车型、OE、图片等信息。仅内部可见。',true)}<p className="text-xs text-slate-600">{missing.length?`发布前待补充：${missing.join('、')}`:'发布所需资料已齐全'}</p>
        ...
        <button type="button" className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-4 py-3 text-sm font-semibold disabled:opacity-40" disabled={dirty||!hasDraft||!confirmed||missing.length>0} onClick={()=>action('publish')}>{product?.status==='published'?'更新线上产品':'发布上架'}</button>
        {product?.status==='published'&&<><a className="block text-center text-sm text-blue-700" href={`https://www.ipackautoparts.com/products/${product.slug}`} target="_blank" rel="noreferrer">查看线上页面 ↗</a><button type="button" className={`${buttonClass} w-full text-red-700`} disabled={dirty} onClick={()=>action('archive')}>下架产品</button></>}{busy&&<p role="status" className="text-sm text-blue-700">正在处理，请稍候…</p>}
