...
function ProductEditor() {
  ...
  return <AdminLayout user={user} onLogout={async()=>{if(!dirty||window.confirm('未保存的修改会丢失，确定退出吗？')){bypassNavigation.current=true;await logout();await router.push('/login');}}}>
    ...
    {loading?<p>正在读取产品资料…</p>:<fieldset disabled={busy||Boolean(id&&!product)} className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_320px] gap-6 min-w-0">
      ...
      </section><section className={panelClass}><h2 className="font-semibold">B2B 询价信息</h2>{input('price_text','价格说明（可选）','留空时显示 Request a quote')}{input('moq_text','起订量（可选）','留空时显示 Contact us')}<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.featured} onChange={e=>change('featured',e.target.checked)}/>推荐产品</label><p className="text-xs text-gray-500">发布后优先展示于 Product 目录，并作为顶部产品菜单的精选候选。菜单展示部分推荐产品；下架后自动移除。</p></section></aside>
    </fieldset>}
