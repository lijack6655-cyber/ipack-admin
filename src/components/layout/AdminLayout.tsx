...
import {
  ...
  BarChart3, Settings, Home, ChevronDown, ChevronRight, Inbox,
} from 'lucide-react';
...
export default function AdminLayout({ children, user, onLogout }: AdminLayoutProps) {
  ...
  const visibleItems = menuItems.map((item) => ({ ...item, submenu: item.submenu?.filter((sub) => canAccessPath(user.role?.name, sub.href)) })).filter((item) => item.submenu ? item.submenu.length > 0 : canAccessPath(user.role?.name, item.href || ''));
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      {sidebarOpen && <button aria-label="关闭导航" onClick={() => setSidebarOpen(false)} className="fixed inset-0 z-30 bg-black/30 md:hidden" />}
      {/* 侧边栏 */}
      <aside
      ...
      >
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-30 flex-shrink-0">
          <div className="flex items-center gap-3">
            <button
              aria-label="切换导航" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(!sidebarOpen)}
