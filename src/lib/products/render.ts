...
export function renderProduct(product: Tables<'products'>, form: ProductInput, options: { preview?: boolean; imageUrls?: Record<string,string> } = {}) {
  ...
  const footer = options.preview ? shell.footer.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi,'') : shell.footer.replace(/main\.js\?v=20260908-leads/g,'main.js?v=20260908-products');
...
<link rel="stylesheet" href="https://www.ipackautoparts.com/assets/css/styles.css"></head>${shell.header.replace(/src="\/assets\//g,'src="https://www.ipackautoparts.com/assets/')}
${options.preview ? '<div style="padding:12px;background:#fff3cd;text-align:center">Draft preview — not published</div>' : ''}
<main><section class="page-hero"><div class="container"><div class="breadcrumb"><a href="/">Home</a> / <a href="/products">Products</a></div><h1>${h(form.title)}</h1><p>${h(form.short_description)}</p></div></section>
